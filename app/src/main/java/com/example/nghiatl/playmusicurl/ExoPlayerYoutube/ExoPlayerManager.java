...
// source: https://androidwave.com/play-youtube-video-in-exoplayer/
public class ExoPlayerManager {
    ...
    public static final YtFile getMaxHeight(SparseArray<YtFile> sparseArray) {
        int maxHeight = sparseArray.valueAt(0).getFormat().getHeight();
        YtFile result = sparseArray.valueAt(0);
        for (int i = 0; i<sparseArray.size(); i++) {
            Log.e("nghia "+i, sparseArray.valueAt(i).getUrl());

            int height = sparseArray.valueAt(i).getFormat().getHeight();
            if (height > maxHeight) {
                maxHeight = height;
                result = sparseArray.valueAt(i);
            }
        }
        return result;
    }
